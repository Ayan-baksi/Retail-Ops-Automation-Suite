// Export.gs
// Exports the Dashboard tab to PDF and saves it to Drive. If export
// fails with a script.external_request permissions error, remove access
// at myaccount.google.com/permissions for this project, then run
// authorizeAllPermissions once from the editor.
...
function exportDashboardToPdf() {
  ...
  try {
    ...
    try {
      ...
      throw new Error('UrlFetchApp.fetch threw an error — almost always the missing "script.external_request" scope. Raw error: ' + fetchErr.message);
    }

    var code = response.getResponseCode();
    log.push('  HTTP response code: ' + code);
    if (code !== 200) {
      throw new Error('Google returned HTTP ' + code + ' instead of 200. Response snippet: ' + response.getContentText().substring(0, 300));
    }

    log.push('Step 5: Building the PDF file blob...');
    var fileName = CONFIG.ORG_NAME + ' SM Dashboard - ' + Utilities.formatDate(new Date(), TZ, 'dd-MMM-yyyy HHmm') + '.pdf';
    var blob = response.getBlob().setName(fileName);
    var blobSize = blob.getBytes().length;
    log.push('  Blob size: ' + blobSize + ' bytes');
    if (blobSize < 1000) throw new Error('The PDF came back suspiciously small (' + blobSize + ' bytes).');
    ...
    log.push('  Saved as: ' + file.getName());

    Logger.log(log.join('\n'));
    ui.alert('Exported ✅', 'Saved to your Drive folder "DeeHub SM Dashboard Exports" as:\n' + file.getName(), ui.ButtonSet.OK);

  } catch (err) {
    Logger.log('EXPORT FAILED\n' + log.join('\n') + '\n\nError: ' + err.message);
    ui.alert('Export failed', 'Something went wrong:\n' + err.message +
      '\n\nFull step-by-step log written to Executions in the left sidebar.', ui.ButtonSet.OK);
  }
}

function getOrCreateExportFolder_() {
  var name = 'DeeHub SM Dashboard Exports';
